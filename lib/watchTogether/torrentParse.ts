// Public WebTorrent-friendly demos seeded by webtorrent.io infrastructure
...
export async function parseMagnetFromTorrentFile(buffer: Uint8Array): Promise<string> {
  // @ts-expect-error - no published types for parse-torrent
  const mod = await import('parse-torrent');
  const parseTorrent = mod.default;
  const toMagnetURI = mod.toMagnetURI;
