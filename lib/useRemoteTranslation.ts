      ...
      try {
        const tokenResponse = await fetch('/api/realtime/translation-token', {
          ...
          body: JSON.stringify({
            ...
            inputTranscriptionEnabled: initialSessionConfig.sourceTranscriptionEnabled,
        ...
        dataChannel.onopen = () => {
          ...
          dataChannel.send(
            JSON.stringify(buildTranslationSessionUpdate(sessionConfigRef.current)),
          );
        };
          ...
          if (!cancelled) {
            void handleRealtimeEvent(event.data, {
              ...
              onInputTranscript: (delta) => {
                setSourceTranscript((current) => appendTranscriptDelta(current, delta));
              },
              onOutputAudio: () => setHasOutputAudio(true),
              onOutputTranscript: (delta) => {
                setTranslatedTranscript((current) => appendTranscriptDelta(current, delta));
              },
              onError: (message) => {
                setError(message);
                setStatus('error');
...
async function handleRealtimeEvent(
  ...
  if (event.type === 'error') {
    ...
    if (eventError && typeof eventError === 'object' && !Array.isArray(eventError)) {
